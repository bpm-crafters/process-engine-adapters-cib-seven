...
class EmbeddedPullUserTaskDelivery(
  ...
  override fun refresh() {
    ...
    if (subscriptions.isNotEmpty()) {
      ...
      logger.trace { "PROCESS-ENGINE-CIB7-EMBEDDED-036: pulling user tasks for subscriptions: $subscriptions" }
      taskService
        .createTaskQuery()
        .initializeFormKeys()
                ...
                try {
                  ...
                  synchronized(deliveredTaskIds) {
                    if (deliveredTaskIds.contains(task.id)) { // if the task was already there, remove it from unprocessed list
                      ...
                      if (!successful) {
                        logger.error { "PROCESS-ENGINE-CIB7-EMBEDDED-038: error processing task ${task.id}, could not mark updated task as processed." }
                      }
                    }
                  }

                } catch (e: Exception) {
                  logger.error { "PROCESS-ENGINE-CIB7-EMBEDDED-038: error delivering task ${task.id}: ${e.message}" }
      ...
      logger.trace { "PROCESS-ENGINE-CIB7-EMBEDDED-039: pull user tasks disabled because of no active subscriptions" }
    }
  }
